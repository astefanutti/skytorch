...
from skytorch.torch.server.server import serve, graceful_shutdown
from skytorch.torch.server.serialization import DEFAULT_CHUNK_SIZE
from skytorch.server.metrics.source import load_metrics_source, list_metrics_source_names


parser = argparse.ArgumentParser(
    description='SkyTorch Server',
...
parser.add_argument(
    '--port',
    type=int,
    default=int(os.environ.get('SKYTORCH_PORT', 50051)),
...
parser.add_argument(
    '--host',
    type=str,
    default=os.environ.get('SKYTORCH_HOST', '[::]'),
...
parser.add_argument(
    '--chunk-size',
    type=int,
    default=int(os.environ.get('SKYTORCH_CHUNK_SIZE', DEFAULT_CHUNK_SIZE)),
...
parser.add_argument(
    '--log-level',
    type=str,
    default=os.environ.get('SKYTORCH_LOG_LEVEL', 'INFO'),
...
parser.add_argument(
    ...
    type=str,
    nargs='*',
    default=os.environ.get('SKYTORCH_METRICS_SOURCES', '').split(',') if os.environ.get('SKYTORCH_METRICS_SOURCES', '') else [],
...
logger.info(f"Starting SkyTorch Server")
