...
"""Generate ATen operator registrations for KPU backend."""
...
def generate_registration_code(operators: list[str]) -> str:
    ...
    for op_name in sorted(operators):
        ...
        lines.append("_kpu_lib_aten.impl(")
        lines.append(f'    "{op_name}",')
        lines.append(f"    _kpu_kernel_fallback_wrapper({op_attr}),")
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="kpu/torch/backend/aten/ops.py",
    ...
    operators = [
        ...
        # in copy.py which handles cross-device copies (kpu<->cpu, kpu<->kpu)
        "neg",
        "neg.out",
        "neg_",
        "abs",
